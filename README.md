...
- `Сделки MOEX` — показать обезличенные сделки, сохранить их в CSV, посчитать VWAP/объем/side imbalance, построить 3-минутный cluster delta график и включить live-обновление прямо в окне;
...
Он опрашивает MOEX ISS с интервалом `0.001` секунды, дописывает в CSV только новые `tradeno` и не дублирует уже сохраненные сделки. Фактическая скорость зависит от ответа MOEX ISS, сети и Windows-планировщика: программа не ставит паузу больше 1 мс, но внешний API может отвечать медленнее. В GUI это кнопки:
...
При временной ошибке чтения QUIK JSON, сети, MOEX ISS или SQLite рекордер не завершается: он выводит сообщение `record-... error` и повторяет попытку через 1 секунду. Изменить паузу можно параметром `--retry-interval`, например `--retry-interval 2`. Параметр `--count` учитывает только успешно сохраненные snapshots/успешные опросы.
